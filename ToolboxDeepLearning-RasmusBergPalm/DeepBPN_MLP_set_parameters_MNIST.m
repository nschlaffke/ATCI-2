...
function [ReducedData Network] = DeepBPN_MLP_set_parameters_MNIST;
...
Network.ActFunction = 'tanh_opt';        %%% 'sigm' (sigmoid), 'tanh_opt' (optimal tanh) or 'reclinear' (ReLU)

% Training parameters
Network.TrainBP.IniWRange = 1.0;            %%% Initial weights in BP training from IniWRange * Gaussian(0,1) / sqrt(fanin)
Network.TrainBP.NumEpochs = 20;             %%% Number of epochs in BP training
Network.TrainBP.BatchSize = 100;            %%% Typical values in [10 100]
Network.TrainBP.LearningRate = 0.1;         %%% Learning rate in BP training
...
Network.TrainBP.DropoutFraction = 0;        %%% Values in [0,1] / Dropout level (http://www.cs.toronto.edu/~hinton/absps/dropout.pdf)
