...
function [net, info] = CNNmat_train_model(net, opts, imdb, varargin)

%%% ERM - Options not used in this example
opts.train = [];
opts.val = [];
opts.errorLabels = {};
opts.continue = false;
...
opts = vl_argparse(opts, varargin);  %%% (optional) Additional options
%opts
...
[net, info] = trainfn(net, opts, imdb, getBatch(opts));

return;


% --------------------------------------------------------------------
function net = cnn_mnist_init(net, opts, varargin)
% --------------------------------------------------------------------
%rng('default');
%rng(0) ;
...
if opts.batchNormalization
  %opts.batchNormalizationLayers
  for l = 1:numel(opts.batchNormalizationLayers)
    net = insertBnorm(net, opts.batchNormalizationLayers(l));
  end
end
...
% --------------------------------------------------------------------
function [net, stats] = cnn_train(net, opts, imdb, getBatch, varargin)
% --------------------------------------------------------------------
